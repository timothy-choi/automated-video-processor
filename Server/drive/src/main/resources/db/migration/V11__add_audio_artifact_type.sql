...
ALTER TABLE artifacts
    ADD CONSTRAINT artifacts_type_check CHECK (artifact_type IN ('THUMBNAIL', 'AUDIO'));
...
CREATE INDEX idx_operations_claim_executable
    ...
    WHERE status = 'QUEUED' AND operation_type IN ('METADATA', 'THUMBNAIL', 'AUDIO_EXTRACTION');
